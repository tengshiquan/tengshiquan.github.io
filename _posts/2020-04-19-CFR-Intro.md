...
累积cumulative regrets为  锤 1, 布 3, 剪 2 , 策略更新为  锤:1/6, 布: 3/6, 剪: 2/6

上面的策略就是 **normalized positive regrets** :  $ \frac{\text{Regret}^+(a)}{\sum(\text{Regret}^+)}   $



下面的算法, 可以通过self-play将预期regret最小化:
...
- For some number of iterations:
  ...
  - Select each player action profile according the strategy profile.  下一个选action的几率是基于regret sum, 不稳定 ;  用平均regret算出来的策略也行
...
核心,  regret的sum的, 然后找出正的部分, 生成下轮策略, 最后所有策略相加平均,求得average strategy across all iterations.  最终会收敛到 **correlated equilibrium 相关均衡**. 



### Counterfactual Regret Minimization

将Regret Minimization**扩展**到信息不完美**序列博弈sequential games**.     
...
- $\pi^{\sigma}(h)$ , **reach probability** of  $h$ with $\sigma$  
- $\pi^{\sigma}(I)$ ,  **reach probability** of $I$ through **all possible histories** $h$ in $I,$ $\pi^{\sigma}(I)=\sum_{h \in I} \pi^{\sigma}(h)$ .
- $\pi_{-i}^{\sigma}(I)$ , **counterfactual reach probability** of $I$ with $\sigma$, except that, we treat current player $i$ actions to reach the state as having probability 1. 除了我们认为当前玩家$i$ 的actions到达这个state的概率是1. 相当于假设玩家i一路选过来.
- **counterfactual** : player i’s strategy was modified to have intentionally played to $I_i$ , 虚拟, 指该玩家的策略被改到有意走到某个信息集.  
- $Z$ , all terminal game histories ,  (root,..., leaf)
- $h \sqsubset z$ , **prefix** , nonterminal game history
- $u_{i}(z)$ , **utility** 
- **counterfactual value**: at nonterminal $h$ , $$v_{i}(\sigma, h)=\sum_{z \in Z, h \sqsubset z} \pi_{-i}^{\sigma}(h) \pi^{\sigma}(h, z) u_{i}(z)$$  
  $\pi_{-i}^{\sigma}(h)$ 表示玩家想走到 $h$ 的概率,  之后则是按照策略组合$\sigma$ 走到结束,  来求期望收益
- **counterfactual regret**: not taking $a$ at history $h$ , $$r(h, a)=v_{i}\left(\sigma_{I \rightarrow a}, h\right)-v_{i}(\sigma, h)$$ 
- **counterfactual regret**: not taking $a$ at  $I$ , $$r(I, a)=\sum_{h \in I} r(h, a)$$  ; 在信息集上,其他一样
- cumulative CFR : $$R_{i}^{T}(I, a)=\sum_{t=1}^{T} r_{i}^{t}(I, a)$$ ,  某个信息集$I$ 上的策略换成a之后的总regret   
  这个定义关键是看信息集$I$ ,因为有的博弈从头到尾, 某个信息集$I$ 就出现一次  
- nonnegative CFR ,  $$R_{i}^{T,+}(I, a)=\max \left(R_{i}^{T}(I, a), 0\right)$$

CFR, 对某个h, 就是在那个node, 玩家没有选a, 而是执行$\sigma$造成的regret, 然后乘以权重:其他人执行$\sigma$ 到这个node的几率.
...
**从本质上说，CFR遍历了大量的游戏子树，在保持历史记录的同时，用每个玩家会到达每个节点（即信息集）的概率向前传递(从root到leaf)，并反传递收益，用于更新父节点的regret，从而更新下一轮的策略。**
