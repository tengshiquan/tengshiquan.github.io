...
将Regret Minimization**扩展**到信息不完美动态博弈. sequential games.     可以被改写成one-time-action normal-form game,即 静态博弈.  但是一般用 game tree 来representation.



##### Kuhn Poker

类似德州的超级简化版游戏.  一共有三张牌1,2,3; 玩家A和B各拿一张牌。

<img src="/img/2020-04-19-CFR-Intro.assets/image-20200515005455936.png" alt="image-20200515005455936" style="zoom: 33%;" />

Kuhn Poker 有 12 个 information sets.   ( 1, 2, 3, b1, b2, b3, p1, p2, p3, 1b, 2b, 3b ) 
...
- cumulative CFR : $$R_{i}^{T}(I, a)=\sum_{t=1}^{T} r_{i}^{t}(I, a)$$  
- nonnegative CFR ,  $$R_{i}^{T,+}(I, a)=\max \left(R_{i}^{T}(I, a), 0\right)$$

CFR, 对某个h, 就是在那个mode, 玩家没有选a, 而是执行$\sigma$造成的regret, 然后乘以权重:其他人执行$\sigma$ 到这个node的几率.
