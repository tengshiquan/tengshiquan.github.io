...
**experience replay** , smooths the training distribution over many past behaviors.  **(s, a, r, s')** 
...
- final hidden layer is fully-connected and con- sists of 512 rectifier units
...
3. error clipping
   - clip the **error term** from the update $$r+\gamma \max _{a^{\prime}} Q   \left(s^{\prime}, a^{\prime} ; \theta_{i}^{-}\right)-Q\left(s, a ; \theta_{i}\right)$$ to be between -1 and 1.
