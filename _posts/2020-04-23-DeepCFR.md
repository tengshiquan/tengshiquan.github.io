...
之前 CFR +  Abstraction , 启发式;   =>  DeepCFR

CFR 是解决大型不完全信息博弈的领先框架。它通过迭代遍历博弈树来收敛到均衡。为了处理非常大的博弈，通常在运行CFR之前应用抽象化。抽象出来的博弈用表格式CFR求解，然后将其解映射回完整的博弈。这个过程可能是有问题的，因为抽象的方面往往是手工操作和领域特定的，抽象算法可能会漏掉博弈中重要的细微差别，而且有一个问题，因为确定一个好的抽象需要对博弈的均衡性有一定的了解。
...
- $h \sqsubset h^{\prime}$ if a **sequence of actions leads** from $h$ to $h^{\prime}$  
...
- probability of a particular action $a$ ,   $\sigma(I, a)$
- $\sigma_{p}$ ,  a strategy for $p$ in every infoset  where $p$ acts. 
- **strategy profile** $\sigma$ is a tuple of strategies
- strategy of every player other than $p$ ,   $\sigma_{-p}$
- $u_{p}\left(\sigma_{p}, \sigma_{-p}\right)$ : **expected payoff** for $p$  , if  $p$  according to $\sigma_{p}$ , other play according to $\sigma_{-p}$ 
- $\pi^{\sigma}(h)=\Pi_{h^{\prime} \cdot a \sqsubseteq h} \sigma_{P\left(h^{\prime}\right)}\left(h^{\prime}, a\right)$ , **reach** probability,  $h$ is reached according to $\sigma$ 
- $\pi_{p}^{\sigma}(h)$ , **contribution** of $p$ to this probability.  $\pi_{-p}^{\sigma}(h)$ contribution of chance and others 
- $\pi_{-p}^{\sigma}(I)=\sum_{h \in I} \pi_{-p}^{\sigma}(h)$ : probability of reaching $I$ if $p$ chooses actions leading toward $I$ ,  chance and others  according to $\sigma_{-p}$
- For $h \sqsubseteq z$ ,   define $\pi^{\sigma}(h \rightarrow z)= \Pi_{h^{\prime} \cdot a \sqsubseteq z, h' \not\sqsubseteq h} \sigma_{P\left(h^{\prime}\right)} (h^{\prime}, a) $
