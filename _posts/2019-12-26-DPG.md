...
[^_^]:4. 论文思路, PG > AC > deterministic Q gradient > DPG > DAC on-policy > DAC off-policy 
...
discounted state distribution: $\rho^\pi(s') = \int_\mathcal{S} \sum_{t=1}^\infty \gamma^{t-1} p_1(s)p(s \to s', t, \pi) \mathrm{d}s$
...
> 一个问题, 就是s的distribution在采样策略以及target策略下, 是不同的. 但总的方向是最大化, 方向大体一致的, 是不是最终能保证target策略on-policy后的 J 也最大的? 
>
...
Off-Policy Actor-Critic (OffPAC)算法, 使用行为策略$\beta(s,a)$来生成trajectories. critic 使用gradient TD learning , off-policy, 从这些trajectories 来拟合 $V^v(s) \approx V^\pi(s)$ ,  actor利用trajectories (off-policy)沿着 公式5的stochastic gradient ascent 方向调整参数$\theta$.  公式5里的 $Q^\pi(s,a)$ 是未知的,  可用 TD-error $\delta_t=r_{t+1} + \gamma V^v(s_{t+1})-V^v(s_t)$ 代替. 因为是用 $\beta$ 来采样, 所以 actor 和 critic都用**importance sampling ratio** $\frac{π_θ(a\vert s)}{β_θ(a\vert s)}$. 
