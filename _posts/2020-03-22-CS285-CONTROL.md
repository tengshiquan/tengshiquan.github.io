...
why is this suboptimal?  但某些随机问题, 给出了action sequence后, 后面出现的状态并不完全由action控制. 不一定出现的每个状态都能遇到最好的action.
...
其中$$J\left(\mathbf{a}_{1}, \ldots, \mathbf{a}_{T}\right)$$是某种函数，并不关心具体是什么, 下面就是求 argmax ,  可以用很多最优化方法,  gradient ascent , L-BFGS ...
...
这里的sample, 并不是说用action与env交互, 而是 sampling  action sequence 然后用model 来evaluate sequence 的 total reward . 
...
- typically use Gaussian distribution for $$p(\mathbf{A})$$
...
2. **Only open-loop planning**  只能应对 open-loop , 可能陷入局部最优
...
- Other reasons – perception, generalization
...
- 下面为了表达简洁,  可见最优的action是state的一个线性函数. 
