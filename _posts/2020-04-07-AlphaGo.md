...
我们使用流水线pipeline分阶段训练network.    

- **SL policy network** $p_\sigma$ ,  专家棋谱训练 ,  梯度质量高, 学得快
- **Rollout policy** $p_\pi$  ,  专家棋谱训练, 快速落子,   rapidly sample actions during rollouts
...
- Trained using 30 million distinct positions from a separate game generated by a **random mix** of $p_{\sigma}(a  \vert  s)$ and $p_{\rho}(a  \vert  s)$ to **prevent overfitting**
...
显然, rollout 使用SL 以及RL网络, 是比直接用Value网络要准的, 但比随机rollout以及fast 网络要准. 评估都是使用1次value网络或者100次rollout .  横坐标是当前state已经下的步数, 显然, 越往后, 网络越准. 也可以看出, 越靠近结束, 随机rollout仍然是不准,这个会导致表现很差, 应该也是限制100个rollout 的原因, 完全随机确实很难收敛.
