...
下面,会用一类特殊的POMDP, 用来当证明的反例. 该类POMDP特性是: 一个observation x可能对应多个state,但每个s只有一个x.  the observations are labels for disjoint partitions of the underlying state space $\mathcal{S},$ i.e., $P\left(X_{i} | s\right)=0$ for all $s \notin S_{i} \subset \mathcal{S},$ and $P\left(X_{i} | s\right)=1$ for all $s \in S_{i} .$ 

> 另外一类POMDP,  s对应多个x, 比如一个显示屏, agent只能读屏幕, s1时候显示1, s2时候显示2, s3时候,显示随机1-5 
