...
<B><mark>而相关均衡则是, 两个人通过某信号, 比如轮流的选择对方最喜欢的, 这样可以达到两个人的收益都一样又都最大化的情况, 这里是3/2, 即两个人一起选B一次, 再一起选S一次</mark></B>
...
##### 奇数定理:  几乎所有的博弈, 都是有奇数个混合纳什均衡.   比如 2个纯,1个混合.
...
如在第五章中研究的具有完全信息的进入博弈的版本中，纳什均衡(在外，斗争) 似乎是不合理的。倘若事实上挑战者进入，在位者的最优反应是"默许"。在完全信息博弈中，我们通过定义**子博弈完美均衡**概念剔除了这种均衡，因为 子博弈完美均衡要求，对于轮到玩家行动的**每个历史**，在给定其他玩家 的策略时，当前行动的玩家的策略是最佳的，无论 玩家们坚持他们的策略时这个历史是否发生。 
...
#### 10.4	Beliefs and sequential equilibrium    信念和序贯均衡

一个战略博弈的纳什均衡可以由两个要求来描述：每个玩家根据自己对其他玩家的信念选择最佳行动，以及每个玩家的信念是正确的. 下面为展开型博弈定义的均衡概念体现了同样的两个要求，并且，就像完全信息展开博弈的子博弈完美均衡概念一样，要求在每一个玩家行动的点上都成立。  
...
我们假设在一个包含多个历史的信息集中，当前行动的玩家会形成一个关于已经发生的历史的信念，我们将这个信念建模为**信息集里的历史的概率分布**（若一个信息集中包含一个单一的历史，唯一可能的信念对该历史赋予概率1）。我们把每个玩家的每个信息集的一个信念集合称为**信念体系 belief system**。
...
展开型博弈中的**信念体系 belief system**是一个函数，它给**每个信息集**分配一个在那个信息集上的**历史的模率分布**。
...
展开型博弈中玩家 i 的**行为策略**是一个函数，为玩家 $i$ 的每一个信息集$I_i$ 分配一个 $A(I_i)$中行动上的概率分布，具有"每个概率分布与其他每个分布独立"的性质。
 ...
 分配概率 1 至单一行动的行为策略与纯策略等价 ; 在所有博弈中，混合策略与行为策略是等价的 , 但行为策略在不完全信息展开博弈问题上使用方便. 
...
- **Sequential rationality (序贯理性)**: Each player's strategy is optimal whenever she has to
  move,given her belief and the other players'strategies.  给定其他人的策略以及自己的信念, 当自己行动时选最优策略.
- **Consistency of beliefs with strategies(信念与策略一致)**: Each player's belief is consistent with the
  strategy profile.    信念正确

序贯理性的要求推广了子博弈完美均衡的要求，后者要求 给定策略组合后, 每个玩家的策略在她行动的每个历史(**node**)之后的博弈部分都是最优的，不管这段历史是否发生。  
在更广泛的博弈里，序贯理性要求 给定策略组合，并给定玩家对已发生的信息集中的历史的信念，如果玩家遵循他们的策略, 每个玩家的策略在她的每个信息集(**info set)**之后的那部分博弈中是最优的，而不管这个信息集是否被达到。  
ps, 这里的达到, 指的是当前这轮博弈, 按照目前的策略组合， 是不是会走到这个节点或者信息集. 就算走不到, 指定的策略也要在万一走到的情况下,是最优的.   
序贯理性只是在自己的信念上,每个信息集最优, 没要求该信念是正确的.  均衡要求信念正确.

下图中的例子,  假设P1 的策略(表示为黑色粗线)为,  起始点选 E，在历史 (C，F)之后选择 $J$ ; P2关于P1的信念P1会2/3选C, 1/3选D; 序贯理性要求:给定P1的策略，P1按照自己的策略行动，这时会造成整个game tree里有些P2的信息集达不到，但是要求P2在所有该自己行动的信息集上的策略应当是最优的.   给定了P2 的信念,在从P2的信息集出发的博弈部分,关于策略 $F$ 的期望盈利是 $\frac{2}{3} \cdot 0+\frac{1}{3} \cdot 1=\frac{1}{3}$ , 关于策略$G$ 的期望盈利是 $\frac{2}{3} \cdot 1+\frac{1}{3} \cdot 0=\frac{2}{3}$ 。 因此,序贯理性要求他选择 $G$ . 在给定 P2 的策略下, 序贯理性也要求 P1 在他的两个信息集上的策略是最优的。P1 在历史(C,F)之后的最优行动是 $J$ ; 如 P2 的策略是 G , 那么 P1 在开始时的最优行动是 $D$ 和 $E$  ; 于是,给定了 P2 的策略 G,  P1 有两个最优策略： $DJ$ 和 $E J$

![326](/img/2020-04-18-Game.assets/image-20201216122021219.png)

其实就是有包含了 信念概率的 反向归纳法. 

记 $(\beta, \mu)$ 为博弈的一个 **评估assessment**  ( $\beta$ **behavioral strategy profile** , $\mu$  **belief system**),  $I_i$ 为玩家$i$的一个信息集. 若$I_i$中每个**历史发生的概率就是玩家的信念** $\mu_i$ (不必是玩家坚持 $\beta$ 时该历史发生的概率) , 并且之后玩家坚持策略组合 $\beta$ , 那么得到终端历史上的一个概率分布,记为  $O_{I_{i}}(\beta, \mu)$ . 
...
如果玩家遵循他们的策略，这类信息集里的每个历史的概率为0，但如果达到这样的信息集，则在那个信息集上行动的玩家必须相信该信息集里某个历史已经发生了.我们通过允许在这样的信息集上行动的玩家在该信息集持有 **任意** 信念来处理这个困难。  显然，如果玩家一直按照策略组合， 则肯定能形成 经历过的信息集 的正确的信念，但没走过的信息集，则任意。  信念正确代表了玩家充分了解双方策略以及博弈本身. 

下面精确地阐述一致(相容)要求，**把信念体系 限制在 仅仅那些 每个玩家坚持自己的策略 将以正概率达到的信息集上**。明确地讲，要求 在这样的(>0概率会达到的)信息集上行动的玩家的**信念指派**给该信息集中**某个历史$$h^*$$的概率**， 应该等于 **按照策略组合 $$h^*$$ 发生的概率** ，以 按照策略组合到达该信息集 为条件的条件概率,  根据贝叶斯公式, 则下式
$$
...
**DEFINITION** 328.1 (**Weak sequential equilibrium**)   An  **assessment 评估** $(\beta, \mu)$ is a **weak sequential equilibrium 弱序贯均衡** , 如果满足:

1. **Sequential rationality 序贯理性**: 每个玩家在其行动的信息集上的策略都是最优的,给定策略组合和已经发生的信息集中的历史的信念.   
...
2. **Weak consistency of beliefs with strategies 信念与策略弱一致性** : For every information set $I_{i}$ reached with positive probability given the strategy profile $\beta,$ the probability assigned by the belief system to each history $h^{*}$ in $I_{i}$ is given by (327.1)  
   给定策略组合, 只考虑每个可以达到的信息集中的历史(概率>0), 信念分配的概率由公式 (327.1) 给出; 即无法达到的信念随意, 可以到达的贝叶斯公式.

弱序贯均衡, 弱体现在, 弱信念一致, 即可到达的信息集的信念是正确的, 走不到的信息集, 信念随意, 只要能凑成让策略最优; 然后序贯理性要在每个信息集上都取最优.

该例, 体现了弱序贯均衡.  

![326](/img/2020-04-18-Game.assets/image-20201216122021219.png)

- 给定 P2 的策略 G，P1 的策略 EJ 是序贯理性的;  给定图上P2的信念以及P1的策略 EJ, P2的策略G是 序贯理性的.  进而, P2的信念与策略组合(EJ,G) 一致, 因为该策略组合走不到P2的信息集, 所以P2任意信念都OK. 万一走到了,P2的策略在P2的信念下也是最优的. 因此,该博弈有一个弱序贯均衡, 其中策略是(EJ,G), 信念是上图中的(或者其他信念使得能选G的信念).  

- 考虑 策略组合(DJ,G) ,  对P2的G, P1是序贯理性的; 给定P2的信念与P1的策略, P2 策略是序贯理性; 但P2信念与策略组合不一致, 该信念是不对的, 因为P1确定选D, 所以相容的信念是P1以概率1选D, 正确的信念使得P2的选F是最优.  所以该博弈没有策略组合是(DJ,G)的弱序贯均衡. 

在完全信息的展开型博弈中，只有一个信念体系是可能的: 每一个玩家在每个信息集上相信"单一适合的历史以概率 1 发生"。所以在完全信息博弈中序贯理性要求的最优就等于子博弈完美均衡要求的最优.  
  **在完全信息的展开型博弈中，任意弱序贯均衡的策略组合就是子博弈完美均衡。**

在一般的展开博弈中，序贯理性的要求意味着，每个玩家的策略在博弈开始时是最优的.  进一步,  信念一致性 要求,  对每个可以到达的信息集里的历史, 每个玩家的信念是正确的. 因此, 如果一个评估是 弱序贯均衡, 那么给定其他人的策略, 那个这个评估中的玩家的策略在博弈起始点是最优的.  即   
**任何弱序贯均衡的策略组合是纳什均衡。**



下面讨论如何求:  为求得博弈的弱序贯均衡﹐我们可以把在完全信息展开型博弈中求**子博弈完美均衡**与求**策略型博弈的纳什均衡**这两种技巧相结合。例如,考虑图326中的博弈。序贯理性要求在弱序贯均衡中, P1在node(C,F)之后选择J。现在我们可以考虑以下两种情况。

- P1选 E 有弱序贯均衡么?  若P1选$E$, P2的belief 可以任意, 所以问题变为, 是否存在P2的策略能否使得P1选E是最优的? P2一直选F,显然不能. 如果P2存在策略,使P1选E是最优的, 那么是否P2存在这样信念, 使得自己的策略是最优的?   可以看到, 如P2最多以2/3选F, P1选E是最优.  任何P2这样的策略是最优的,如果P2相信历史是C的概率是1/2 (P2使用混合策略,使得P1的收益<=2,求得<=2/3选F,同时,自己的信念要使得自己取F或G收益一样,求得概率1/2) ; 若P2相信历史是C的概率>=1/2,则P2以概率0选F是最优的.  (任意信念,凑出最优)
- P1选C或者D有弱序贯均衡么? 设P1选C以p, 选D以q. 由于一致性的要求, P2的信念, 分配概率 $p /(p+q)$ to $C$, $q /(p+q)$ to $D$ . 那么对P2的收益, $G$ 是最优的,  if $p>q$;   $F$ is optimal if $p<q$ ; 任何混合都是最优,  if $p=q .$ 
  现在, 若 P2 最优选 $G$, P1的最优策略为 选D或E, 即$p=0$, 矛盾;  若 P2 最优选 $F$, 则P1最优选C或者E,也矛盾. 如果P2最优是混合策略(概率>0),  则要求P1的p=q>0, 由于P1选D严劣于选E, 矛盾; 所以不存在.

另外一个找 弱序贯均衡的方法是, 先找出所有的纳什均衡, 然后再验证 是否与弱序贯均衡的条件匹配.



EXAMPLE 330.1 (Weak sequential equilibria of entry game) 

![317](/img/2020-04-18-Game.assets/image-20201216161810439.png)

![322](/img/2020-04-18-Game.assets/image-20201216170146824.png)

Example 317.1 有两个纯策略均衡 (Unready, Acquiesce) and (Out, Fight).    
考虑第一个均衡, 一致性要求P1在其信息集上相信历史是Unready，使得Acquiesce最优。显然只要P2使用Unready,的确该信念是对的, 所以 (Unready, Acquiesce) 是一个弱序贯均衡,P2的信念是Unready.  
考虑第二个均衡,  不管P2的任何信念, Fight 对P2都不是最优action. 所以没有任何评估使得(Out, Fight) 是弱序贯均衡.

 Examples 314.2 (BoS as an extensive game) and 315.1 (the card game) 都没有某些信息集无法走到的纳什均衡策略, 所以出现在弱序贯均衡的策略就是纳什均衡. 

为什么是 **"弱"**序贯均衡? 一致性条件对以正概率达成的信息集的限制，在一些博弈中产生了较大的均衡评估集，其中一些评估并不能处于稳定状态。

![331](/img/2020-04-18-Game.assets/image-20201222191757059.png)

例如，考虑图 331.1中的博弈，是图323.1中进入博弈的一个变体，在这个博弈中，无论P2的行动如何，对于P1来说，Ready比Unready更好。这个博弈有一个弱序列均衡，其中P1的策略是Out,P2是F，P2相信历史是Unready。在这个均衡中，P2相信P1选择了Unready，但Unready对P1来说严劣于Ready的, 因此P2的信念似乎并不合理。 

具有较强一致性条件的均衡，缩小了某些博弈中均衡结果的集合。有一个概念，序贯均衡，被广泛使用。然而，它所施加的一致性条件并不直接，该概念无论如何也不能消除所有不合理的均衡--例如，它不能消除图331.1中P1选择Out的博弈的均衡。  
此外, 在很多博弈, 弱序贯均衡集并不大, 而且不包含不合理的评估.  所以下面就用弱的.

EXERCISE 331.2 (Weak sequential equilibrium and Nash equilibrium in subgames)   
图331.1中博弈的变体, 图 332.1, P1的动作被拆成了两步, 证明该博弈有一个弱序贯均衡,其在历史In之后的subgame不是纳什均衡.

![332](/img/2020-04-18-Game.assets/image-20201222211249349.png)

对P1而严, R严优于U. 所以P1的策略里面, U的概率为0;  考虑让P1选Out, 则P2的信念以为P1<=1/2选R,P2选F, P1选Out; P2信念以P1选R,自己选A,P1选In,R. 

考虑这样的评估：P1的策略是(Out,R)，P2是F，P2的信念(In,U)概率1。每个玩家的策略是序贯理性.因为当P1遵循自己策略时，P2的信息集没有达到。所以此评估是一个弱序贯均衡。
该评估在历史 In之后的subgame中,不构成subgame的Nash均衡，因为当P1选择R时，P2的行动F不是最优的（P2的行动F是最优的，因为她相信历史是（In,U），如弱序贯均衡中)



#### 10.5	Signaling games   信号博弈

在许多交互中，信息是 "**不对称的 asymmetric**"：一些人被告知影响到每个人的变量，而一些人则不被告知。 有一些情况, 知情方有机会在不知情方采取影响所有人的行动之前，采取不知情方所观察到的行动。在某些情况下，知情方的行动可以发出信息 "信号"。  
例如，雇主可以观察到一个求职者是否有大学学历，但不能观察到她的能力，一个能力强的人可能会以较低的成本获得一个学位，而一个能力弱的人可能会以较高的成本获得学位，那么，一个人有学位的事实可能会向雇主发出信号，表明她具有较高的能力。

一般的信号博弈**Signaling game**:  "发送者sender"被告知一个与大家都相关的变量。知情的玩家（"发送者sender"）采取了一个被不知情的玩家（"接收者receiver"）观察到的行动，然后接收者则采取了一个影响双方的行动。根据信号和接收者的行动对双方的影响方式，发送者可能希望限制或扭曲其信号所传达的信息。

这样的情况可以建模为展开型博弈，其中发送者有若干个可能的"类型"，每一个类型对应于其所获知的变量的一个值。他观察到的值，也就是他的类型，由随机的方式确定。接收者观察不到发送者的类型，而是能看到发送者所采取的行动，然后接收者自己再采取行动。

EXAMPLE 332.1(Entry as a signaling game) 挑战者P1是强人的几率p, 弱1-p. 0 < p<1; P1知道自己的类型，P2并不知道.  P2可以观察到P1是否 采取 Ready, 但无法知道其类型. 

![image-20201222225418506](/img/2020-04-18-Game.assets/image-20201222225418506.png)

<img src="/img/2020-04-18-Game.assets/image-20200511120127099.png" alt="image-20200511120127099" style="zoom:50%;" />

P1由chance决定类型, 两种类型都有两个动作，即Ready和Unready，因此P1有四种策略; 而P2有两个信息集，每个信息集都有两个动作（A和F），因此也有四个策略。

现在求这个博弈的 纯 弱序贯均衡。依次考察,P1的策略.   首先观察到, 弱的P1喜欢 Unready , 严优.

1. 强 P1 选 Ready, 弱P1,Unready, 给定P1的策略,  P2的两个信息集都会走到. 由于一致性要求, 上面那个信息集,P2相信历史是(Strong,Ready), 所以选A; 下面的, P2相信是(Weak,Unready), 选F.  得到一个弱序贯均衡. 
2. 强 P1 选 Unready, 弱P1,Unready;   P2, 下面的信息集, 相信(Strong,Unready)概率是p, (Weak,Unready)概率1-p , 因此, 选A收益2p, 选F收益1-2p, 所以,  p>=1/4 选A, 否则选F. 
   1. 假设p>=1/4 , P2 选 A 来应对 Unready,  P1的收益是5, P1不会偏离.  因此p>=1/4 有弱序贯均衡, P1选Unready, P2选A, P2对 Reay 类型的P1 ,可以有任何信念,并根据其信念选A或者F.
   2. 假设p<=1/4 , P2选F来应对.  选Unready的 强P1收益是3;  若P1偏离,  如果P2 F, P1收益2, 如果P2 A, P1收益4, 所以为了均衡(不让P1偏), P2 必须 选F 来应对 Ready的P1 . 若P2相信 Ready的P1有>=3/4的概率是弱的, 那么F是最优的. 这样的信念是可能的. 因为P1是选择的Unready, 上面的信息集走不到. 所以p<=1/4 有弱序贯均衡, P1选Unready, P2选F,并且如果P2观察到P1采取Ready,其信念为>=3/4 P1是弱的.
...
- **Separating equilibrium 分离均衡**   每种类型的发送者选择不同的行动(上例,强P1选Ready,弱P1选Unready), 接受者通过观察发送者的行动, 知道了发送者的类型

- **Pooling equilibrium 混同均衡**   所有类型的发送者采取相同的action(上例的第二个均衡,P1都是Ready), 所以发送者没有给接受者有关类型的线索.

如果sender的类型多于两个, 上面两种均衡的混合分组可能存在. 
...
弱序贯均衡是序贯均衡的一个变体.  弱序贯均衡又被称为 **评估均衡 assessment equilibriuim**, **弱完美贝叶斯均衡 weak perfect Bayesian equilibrium**.
...
纳什均衡 是对一种稳态 建模, 指的是，每个玩家通过与各种对手的博弈经验，形成了有关其他人行动的正确信念,  并根据这些信念选择自己的行动。
在本章和下一章中，我们从不同的角度来研究玩家的行为。我们考虑每个玩家对其他玩家的行动所形成的信念，不是从他的经验出发，而是从他对博弈的分析出发，来考虑其意义。
这一章重点讨论的二人严格竞争博弈 Strictly Competitive Game，在这种游戏中，玩家的利益是截然相反的。在这种博弈中，一个简单的决策程序导致每个玩家选择一个纳什均衡行动。



#### 11.1  Maxminimization  最大最小化

你**第一次**面临博弈;**对于你的对手**将采取什么行动**没有任何信念**。你应该怎样选择你的行动呢?给你一个非常**保守的方案** : 将你的每一个行动与最差的结果联系起来(因为其他玩家的行动各不相同), 然后在这些最差的结局中找出一个最好的对应的行动。这种方法称为 "**最大最小化maxminimization**"法。

例, 图362, 只有P1的收益, P2的收益与P1的行动无关, 这里只考虑纯策略,  P1选T,最差收益是0, 选B,最差是1, 因此纯策略,最大最小法,找到的是B.

![image-20201223110637380](/img/2020-04-18-Game.assets/image-20201223110637380.png)

许多 最大最小化的例子涉及混合策略，所以从一开始 就定义具有vNM偏好的策略博弈的概念，尽管这些想法并不取决于玩家的随机。 

令 $u_{i}$ 为玩家的伯努利收益, 代表玩家对 lottery的偏好,  记 $U_{i}(\alpha)$ 为玩家关于混合策略组合$\alpha$ 的收益, 对任何给定的玩家混合策略组合$\alpha_{i}$ , 对其他人可能的任何混合策略$\alpha_{-i}$,  玩家$i$ 的最低收益为
$$
\min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)
$$
一个最大最小化混合策略就是 使得最差收益最大化的混合策略.

DEFINITION 362.1 (**Maxminimizing mixed strategy** in strategic game with **vNM** preferences) A maxminimizing mixed strategy for player $$i$$ in a strategic game with vNM preferences is a mixed strategy that solves the problem
$$
\max _{\alpha_{i}} \min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)
$$
where $$U_{i}(\alpha)$$ is player i's expected payoff to the strategy profile $$\alpha$$.  
If $$\alpha_{i}^{*}$$ is a **maxminimizing mixed strategy** for player $$i$$ ,  then $$\min _{\alpha_{-i}} U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}\right)$$ is her **maxminimized payoff 最大最小化收益**.

注意, 等价的, $$\alpha_{i}^{*}$$ 是一个 **最大最小化解 maxminimizer** if and only if $$\min _{\alpha_{-i}} U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}\right)=$$ $$\max _{\alpha_{i}} \min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)$$

换句话说，在(非常悲观的)假设下，玩家$i$的最大最小化策略使她的预期收益最大化. 悲观假设为,无论他做什么，其他玩家都会以最小化他的预期回报的方式行动。

另一个角度看到 最大最小化策略, 若一个混合策略 $$\alpha_{i}$$ **保证guarantees** 收益最少为$$\bar{U}_{i}$$ , 保障收益 , 
即, 对其他人的每个混合策略$$\alpha_{-i}$$ , 有 $$U_{i}\left(\alpha_{i}, \alpha_{-i}\right) \geq \bar{U}_{i}$$ 

图362.1, P1的策略T 收益保障值最大为0, 选B收益保障值1.  

玩家的最大最小化混合策略, 保证了其 最大最小化收益. 进而, 没有一个策略可以保证其得到更高的收益.  即,  $$\alpha_{i}^{*}$$ 是最大最小化策略 , 那么 $$\min _{\alpha_{-i}} U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}\right) \geq \min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)$$ 

**就是说, maxminimizer是所有策略里面,  玩家能获得最大的回报保障.** 

玩家可能可以获得更好的收益,使用混合策略而不是纯策略,看下例如何找到该混合策略.

EXAMPLE 337.1   重要, 求maxminimizers的例子.  

<img src="/img/2020-04-18-Game.assets/image-20200508011853619.png" alt="image-20200508011853619" style="zoom:33%;" />

显然, 如果P1 选T, 收益保障是-1, 选B, 收益保障是-1.  然后, P1在T和B中随机化, 收益可能更好一点. 比如1/2选T, 1/2选B, 则玩家2固定选择L,P1的预期收益是1/2, 玩家2选择R,P1的预期收益是0;   令$p$ 为P1选择T的概率.  下图中两条线分别表示**P2分别选择L,R时候,P1的收益**. 黑色的倒V线表示P1所能获得的被P2针对下的收益保障,即 $\min_{\alpha_2}u_1(\alpha_1,\alpha_2)$  , 而P1需要max这个保障, 所以选择p=2/5.  正好是用BR相交法求出的混合策略均衡的值.

<img src="/img/2020-04-18-Game.assets/image-20200508012459016.png" alt="image-20200508012459016" style="zoom: 33%;" />

在此示例中，P1的**最大最小化混合策略**具有以下性质：无论P2选择L还是R，它都会使P1自己获得相同的收益。**注意，这种indifference 与混合策略纳什均衡中的indifference 不同**，在混合策略纳什平衡中，P1的混合策略使P2的每个动作获得相同的收益。
...
**LEMMA 338.1**  **对策略博弈, 任何纳什均衡策略的收益都大于等于 最大最小化策略的收益**.  *The payoff of each player in any Nash equilibrium of a strategic game is at least equal to her maxminimized payoff*. 

Proof.  

记 $$\alpha^{*}$$ 是纳什均衡策略,   对玩家 $i$,  对其任意混合策略 $$\alpha_{i}$$, 由纳什均衡定义,有 $$U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}^{*}\right) \geq U_{i}\left(\alpha_{i}, \alpha_{-i}^{*}\right)$$ 	, so  $$U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}^{*}\right) \geq  \max_{i} U_{i}  \left(\alpha_{i}, \alpha_{-i}^{*}\right)$$ 

又由于   $$U_{i}\left(\alpha_{i}, \alpha_{-i}^{*}\right) \geq \min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)$$  , so 
$$
U_{i}\left(\alpha_{i}^{*}, \alpha_{-i}^{*}\right) \geq \max _{\alpha_{i}} \min _{\alpha_{-i}} U_{i}\left(\alpha_{i}, \alpha_{-i}\right)
$$
...
在许多博弈中，玩家没有充分的理由去相信其他人将采取使其收益最小化的行动。可是在玩家的利益完全对立的**两人博弈**中，这个假设是合理的 . 

先看纯策略的 零和博弈.

在一个**双人博弈**中，如果一个玩家倾向于某一结果a而不是另一结果b时，另一个玩家倾向于b而不是a，那么玩家的利益是截然相反的。如果我们把注意力限制在纯策略上，那么就有了下面的定义，为了方便，假设玩家的名字是 "1 "和 "2"。

DEFINITION 339.1 (**序数偏好的严格竞争策略型博弈 Strictly competitive strategic game with ordinal preferences**)  一个策略型博弈是**严格竞争**的, 如果有两个玩家, 并且有
$$
u_{1}\left(a_{1}, a_{2}\right) \geqslant u_{1}\left(b_{1}, b_{2}\right) \quad \text { if and only if } \quad u_{2}\left(b_{1}, b_{2}\right) \geqslant u_{2}\left(a_{1}, a_{2}\right)
$$
其中 $\left(a_{1}, a_{2}\right)$ 和 $\left(b_{1}, b_{2}\right)$ 是行动对;   主要意思就是, 两个人的收益函数, 偏好的方向是相反的.

注意，如果 $u_{1}$ 是描述严格竟争博弈中P1偏好的收益函数,那么收益函数$-u_1$ 则描述了玩家2 的偏好。  
也就是说，在任何严格竟争博弈中,**存在**玩家的收益函数$u_1$ 和 $u_2$,  使得对于任意的行动组合 $\left(a_{1}, a_{2}\right)$ 成立 $u_{1}\left(a_{1}, a_{2}\right)+u_{2}\left(a_{1}, a_{2}\right)=0$ 。出于这个原因, **严格竞争博弈**有时叫作"**零和 zerosum**" 博弈。   
 零和博弈是严格竞争博弈在收益函数为0时的特殊情况. 因为收益函数是序数偏好, 其实 (-1,2) 这种的也是严格竞争的. 但肯定可以设计出sum为0的收益函数.

囚徒困境,BOS都不是严格竞争;  匹配硬币是的. 

下例,  也是**严格竞争, 纯策略的时候**.  P1喜欢(B, R) > (T, L) > (B, L) > (T, R) , 玩家2正好**相反**. 但**考虑混合策略,则不是严格竞争**.
...
$$
U_{1}\left(\alpha_{1}, \alpha_{2}\right) \geqslant U_{1}\left(\beta_{1}, \beta_{2}\right) \quad \text { if and only if } \quad U_{2}\left(\beta_{1}, \beta_{z}\right) \geqslant U_{2}\left(\alpha_{1}, \alpha_{2}\right)
$$
...
**当允许混合策略时,为严格竟争的任何博弈，在限于纯策略时 也是严格竞争的,但是反之不成立。**例如,考虑图339.1中的博奕，现在将方框内的数字解释为伯努利盈利。P1对于如下两种结果感觉没有差别 : 一种是结局 ( $T, L$ ); 另一种是“( $T, R$ ) 以概率 $\frac{3}{5}$ 发生和$(B, R)$以概率$\frac{2}{5}$ "的随机结局, 因为$\frac{3}{5} \cdot 0+\frac{2}{5} \cdot 5=2$,  可是玩家2 对于这两个结局感觉有差异[他关于(T,L)的盈利是 1，而相应于那个随机结局，他的期望盈利是 $\left.\frac{3}{5} \cdot 5+\frac{2}{5} \cdot 0=3\right]$ , 说明关于这个策略对, 两个人的偏好关系不相反.



EXERCISE 340.1(Determining strict competitiveness) 判断下图 340.1, 是否是严格竞争, 是否是混合策略严格竞争?

![image-20201223175522253](/img/2020-04-18-Game.assets/image-20201223175522253.png)

左边,右边 对纯策略, 显然都是严格竞争.

要判断是否可以 混合策略严格竞争,  标准是存在 $\pi$ and $\theta>0$ such that $-u_{1}(a)=\pi+\theta u_{2}(a)$ for every pure outcome $a$  (? 怎么推导)

对左边,存在   $-u_{1}(a)= -\frac{1}{2}+\frac{1}{2} u_{2}(a)$ ; 左边不存在, 所以左边是 混合策略严格竞争.
...
- **混合策略严格竞争博弈中,  纳什均衡盈利 = 最大最小化盈利**



下面证明，**在具有混合策略纳什均衡的严格竞争博弈中，这两个盈利是相同的** ,事实上有:

在具有混合策略纳什均衡的严格竞争博弈中，一对混合策略是混合策略纳什均衡，当且仅当每个玩家的策略是**最大最小化解maxminimizer**。

先证明一个初步的结果,  令 $U_2 = - U_1$,  对任何函数, 有 $$\min _{x} f(x)=-\max _{x}(-f(x))$$ ( 如图 367.2 极值点关于x轴镜像).  因为 $U_{2}=-U_{1}$ , 所以

$$
\max _{\alpha_{2}} \min _{\alpha_{1}} U_{2}\left(\alpha_{1}, \alpha_{2}\right)=\max _{\alpha_{2}}\left(-\max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)\right)
$$

Or

$$
\max _{\alpha_{2}} \min _{\alpha_{1}} U_{2}\left(\alpha_{1}, \alpha_{2}\right)=-\min _{\alpha_{2}} \max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)
$$

![image-20201223185146423](/img/2020-04-18-Game.assets/image-20201223185146423.png)

PROPOSITION 341.1 (**严格竟争博弈中的纳什均衡策略和最大最小化解**) 考虑具有 vNM 偏好的严格竟争策略型博弈。令$U_1$ 为描述P1偏好的期望收益函数,令$U_{2}=-U_{1}$   
...
- b. 如果$$\max _{\alpha_{1}} \min _{\alpha_{2}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)=\min _{\alpha_{2}} \max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)$$ , [特别地,如果博弈有一个混合策略纳什均衡,那么 条件a 满足], $$\alpha_{1}^*$$ 是P1的最大最小化解, $$\alpha_{2}^*$$ 是玩家2的最大最小化解,  那么($$\alpha_{1}^{*}, \alpha_{2}^{*}$$)是混合策略纳什均衡。

证明:略.

**首先**，结果的a部分意味着在**严格竞争博弈**中，每个人的混合策略纳什均衡**收益是唯一**的。

COROLLARY  342.1  严格竞争博弈中的**每个混合策略纳什均衡产生相同的一对期望收益**。 Every Nash equilibrium of a strictly competitive game yields the same pair of payoffs.

如同我们已经着到的，在非严格竞争博弈中，不是所有的纳什均衡都必然产生相同的收益组合(例,考虑 BoS)  不过BoS只有一个混合策略均衡.

**其次**，假设 $\left(\alpha_{1}, \alpha_{2}\right)$  和 $\left(\alpha_{1}^{\prime}, \alpha_{2}^{\prime}\right)$ 都是严格竞争博弈中的混合策略纳什均衡。 那么由命题341.1 的a部分可知，策略$\alpha_{1}$ 和$ \alpha_{1}^{\prime}$ 是P1 的最大最小化解，$\alpha_{2}$ 和$ \alpha_{2}^{\prime}$ 是玩家2 的最大最小化解。然而由 b部分的结果可知，$\left(\alpha_{1}, \alpha_{2}^{\prime}\right)$和$\left(\alpha_{1}^{\prime}, \alpha_{2}\right)$ 都是博弈的混合策略纳什均衡。也就是说，我扪有下述结果。
...
博弈 BoS 显示，非严格竞争博弈的纳什均衡不一定可以互换。 不过BoS只有一个混合策略均衡.
...
在非严格竞争博弈中, 命题 341.1 中的 a 部分一般不成立, 并且玩家的均衡策略也没有这个性质.如下面习题.

重要例子.

EXERCISE 370.2 (Maxminimizers in BoS) For the game BoS (Figure 19.1 ), 求最大最小化解maxminimizer;  对于每个混合策略均衡, 证明没有一个玩家的混合策略可以确保他的均衡收益.

![image-20201210212437764](/img/2020-04-18-Game.assets/image-20201210212437764.png)

P1的最大最小化解是 1/3 选Bach, 使得自己的收益保障是2/3, 即不论P2怎么动作, P1最少收益2/3, 从自己的角度出发; P2 的最大最小化解是 2/3 选Bach.   
P1的混合策略均衡为2/3选Bach, 使得P2的收益永远是2/3; P2的混合策略是1/3选Bach. 两个人的混合策略均衡收益都为2/3     
显然, 任何一方的纯策略均衡无法保证玩家的均衡收益.  
任何一方单独使用均衡策略无法保证收益>=2/3 , 例如P1走混合均衡, P2选S, 则P1收益为1/3,P2的收益2/3. 

所以, 均衡策略是在让对手无法变好的情况下, 再走自己的最优; 而最大最小化是尽量保障自己的收益. 出发点不一样.




然而, 对任何博弈(不限于严格竞争),下式成立:

$$
\max _{\alpha_{1}} \min _{\alpha_{2}} U_{1}\left(\alpha_{1}, \alpha_{2}\right) \leq \min _{\alpha_{2}} \max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)
$$

即, P1 能保障自己的最高收益, 不会 大于 P2能保证的P1的最低收益.  

证明, 对任何混合策略 $\alpha_{1}^{\prime}$  , 有 $$U_{1}\left(\alpha_{1}^{\prime}, \alpha_{2}\right) \leq \max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)$$ ,  对所有$\alpha_{2}$成立,  
hence,  $$\min _{a_{2}} U_{1}\left(\alpha_{1}^{\prime}, \alpha_{2}\right) \leq \min _{\alpha_{2}} \max _{\alpha_{1}} U_{1}\left(\alpha_{1}, \alpha_{2}\right)$$    
因为左边的所有值都成立, 那么左边取最大化的值, 不等式也成立. 



EXERCISE 344.1(Equilibrium in strictly competitive games)证明或给出一个反例，说明如果在严格竞争博弈中，玩家1的均衡报酬是v，那么任何给玩家1带来v报酬的策略对都是一个均衡。

不成立,  下图的严格竞争博弈,  TL 是一个纳什均衡, P1的唯一均衡收益为0. 但BR虽然使得P1收益为0, BR不是均衡.

![image-20201223231326034](/img/2020-04-18-Game.assets/image-20201223231326034.png)
