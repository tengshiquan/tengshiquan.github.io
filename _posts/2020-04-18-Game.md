...
下图中的例子,  假设P1 的策略(表示为黑色粗线)为,  起始点选 E，在历史 (C，F)之后选择 $J$ ; P2关于P1的信念P1会2/3选C, 1/3选D; 顺序理性要求:给定P1的策略，P1按照自己的策略行动，这时会造成整个game tree里有些P2的信息集达不到，但是要求P2在所有该自己行动的信息集上的策略应当是最优的.   给定了P2 的信念,在从P2的信息集出发的博弈部分,关于策略 $F$ 的期望盈利是 $\frac{2}{3} \cdot 0+\frac{1}{3} \cdot 1=\frac{1}{3}$ , 关于策略$G$ 的期望盈利是 $\frac{2}{3} \cdot 1+\frac{1}{3} \cdot 0=\frac{2}{3}$ 。 因此,顺序理性要求他选择 $G$ . 在给定 P2 的策略下, 顺序理性也要求 P1 在他的两个信息集上的策略是最优的。P1 在历史(C,F)之后的最优行动是 $J$ ; 如 P2 的策略是 G , 那么 P1 在开始时的最优行动是 $D$ 和 $E$  ; 于是,给定了 P2 的策略 G,  P1 有两个最优策略： $DJ$ 和 $E J$
...
如果玩家遵循他们的策略，这类信息集里的每个历史的概率为0，但如果达到这样的信息集，则在那个信息集上行动的玩家必须相信该信息集里某个历史已经发生了.我们通过允许在这样的信息集上行动的玩家在该信息集持有**任意**信念来处理这个困难。  显然，如果玩家一直按照策略组合， 则肯定能形成 经历过的信息集 的正确的信念，但没走过的信息集，则任意。
...
1. **Sequential rationality 顺序理性**: 每个玩家在其行动的信息集上的策略都是最优的,给定策略组合和已经发生的信息集中的历史的信念.   
   ...
   对每个人, 在该评估下, 预期收益是最好的.  换了策略，信念是不是要变？
