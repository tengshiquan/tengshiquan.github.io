...
- how?  just solve $$\theta \leftarrow \arg \max _{\theta} Q_{\phi}\left(\mathbf{s}, \mu_{\theta}(\mathbf{s})\right)$$  是个最优化问题, 找到能使 $Q(s,\mu)$最大的$\theta$ . 链式法则,  $$\frac{d Q_{\phi}}{d \theta}=\frac{d \mathbf{a}}{d \theta} \frac{d Q_{\phi}}{d \mathbf{a}}$$ , 相当于梯度是两个网络的梯度相乘. 
