...
这里的积分用期望公式的角度去看待 , 按照某个策略走得到的$\tau$分布, 再之上求期望.
...
初始分布和转移概率本身都与参数$\theta$ 并不相关,  $r(\tau)$ 也只与env相关. 
...
- 再利用因果性, 每个action只是在产生作用后才有影响, 所以内部的sum从t开始
...
-  证明,  subtracting a baseline is *unbiased* in expectation!   只要这个baseline是const,  有没有baseline的期望与之前一样
...
- 可以引入因果性, 一定程度上缓解这个问题. 
- 做一个近似, 不再是原来的PG算法, 将上式最后一部分近似成1, 可以得到  policy iteration algorithm,  然后仍然能improve策略.
...
- off-policy policy gradient: 
  ...
  =\frac{1}{N} \sum_{i=1}^{N} \sum_{t=1}^{T} \color{red}{ \frac{\pi_{\theta^{\prime}}\left(\mathbf{s}_{t, t}\right)}{\pi_{t}\left(\mathbf{s}_{i, t}\right)} }\frac{\pi_{\theta^{\prime}}\left(\mathbf{a}_{i, t} | \mathbf{s}_{i, t}\right)}{\pi_{\theta}\left(\mathbf{a}_{i, t} | \mathbf{s}_{i, t}\right)} \nabla_{\theta^{\prime}} \log \pi_{\theta^{\prime}}\left(\mathbf{a}_{i, t} | \mathbf{s}_{i, t}\right) \hat{Q}_{i, t}
  $$
