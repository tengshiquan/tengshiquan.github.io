...
2. loop K 
   ...
   2.  set $$\phi \leftarrow \arg \min _{\phi} \frac{1}{2} \sum_{i}\left\|Q_{\phi}\left(\mathbf{s}_{i}, \mathbf{a}_{i}\right)-\mathbf{y}_{i}\right\|^{2}$$ <=  Q-y  **bellman error**
...
- 问题在于上式的 拟合目标y并不是一个常数, 该值depends on 网络的参数, 所以并不是bellman error. 
